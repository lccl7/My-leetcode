//The idea is to use a Trie to build a prefix tree for words to simplify the search
//and do DFS to search all the possible strings.
//For Trie, 26 pointers to point the sub-strings and a bool leaf to indicate
//whether the current node is a leaf (i.e. a string in words)
//and also idx is used to save the index of words for the current node.
//For DFS, just check if the current position is visited before (board[i][j]=='X'),
//if so, return, check if there is a string with such prefix (nullptr == root->children[words[idx][pos]-'a']),
//if not, return; otherwise, check if the current searched string is a leaf of the trie (a string in words),
//if so, save it to res and set leaf of the trie node to false to indicate such string is already found.
//At last, move to its neighbors to continue the search. Remember to recover the char [i][j] at the end.
